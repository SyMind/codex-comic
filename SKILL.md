---
name: comic-workflow
description: 根据用户创意制作 3:4 竖版中文手绘教育漫画：编写按页/分镜脚本，建立角色、场景、风格设定，使用 Codex Chrome 插件在 ChatGPT 生成参考图和最终漫画页，并把提示词、图片和清单保存到用户指定目录。适用于漫画、分镜、条漫、角色/场景设定图和完整漫画图片生成工作流。
---
...
- 图片生成必须通过 Codex Chrome 插件操作 ChatGPT；本地只负责脚本、提示词、文件归档和校验。
...
4. 生成参考图：按角色/场景逐个新开 ChatGPT 会话，下载到 `references/`。
5. 生成漫画页：为每一页打开独立的新 ChatGPT 会话，上传相关参考图，提交该页提示词；可同时打开多个新对话并发生成多页，但每个对话只负责一页、只产出一张最终图；下载到 `pages/page-XX.png`。
6. 收尾：确认文件齐全、图片可打开、比例接近 3:4、角色/场景连续性正确；更新 `manifest.json` 并向用户说明结果位置。

## 阶段参考

- 浏览器操作和下载：`references/chatgpt-chrome-workflow.md`
